...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Version 4 repair tool: retag sidecars currently marked ai:caption/low-quality "
            "using a stronger Florence-2 vision model, independent of the curated tag list."
        )
    )
    ...
    p.add_argument("--model", default=DEFAULT_MODEL, help="Florence-2 model. Try microsoft/Florence-2-large-ft for higher quality.")
    p.add_argument("--task", default=DEFAULT_TASK, help="Florence task prompt, e.g. <MORE_DETAILED_CAPTION>, <DETAILED_CAPTION>, <OD>.")
    ...
    p.add_argument("--low-quality-tag", default=LOW_QUALITY_TAG, help="Tag identifying sidecars to repair.")
    p.add_argument("--prospective-prefix", default="prospective", help="Write tags as ai:<prefix>/<term>.")
    p.add_argument("--max-tags", type=int, default=4, help="Maximum prospective tags to add per image.")
    p.add_argument("--min-term-len", type=int, default=3)
    p.add_argument("--update-caption", action=argparse.BooleanOptionalAction, default=True, help="Replace the bad caption with the Florence caption.")
    p.add_argument("--keep-low-quality-tag", action="store_true", help="Keep ai:caption/low-quality instead of removing it.")
    p.add_argument("--limit", type=int, default=0, help="Limit number of sidecars to process. 0 means no limit.")
    return p.parse_args()
...
def ensure_deps() -> None:
    if torch is None or AutoModelForCausalLM is None or AutoProcessor is None:
        raise RuntimeError(
            ...
            "  uv add torch torchvision transformers pillow tqdm accelerate sentencepiece protobuf pillow-heif timm einops"
        )
...
def prepare_heic_with_sips(path: Path, max_side: int) -> Optional[Image.Image]:
    if shutil.which("sips") is None:
        return None
    with tempfile.TemporaryDirectory(prefix="florence_heic_sips_") as td:
...
def prepare_heic_with_ffmpeg(path: Path, max_side: int) -> Optional[Image.Image]:
    if shutil.which("ffmpeg") is None:
        return None
    with tempfile.TemporaryDirectory(prefix="florence_heic_ffmpeg_") as td:
        out = Path(td) / "converted.jpg"
        res = subprocess.run(
            [
                "ffmpeg",
...
class FlorenceRunner:
    def __init__(self, model_name: str, device: str, verbose: bool):
        ...
        if device == "cuda":
            ...
            # MPS float16 may work for some Florence builds but float32 is safer.
            dtype = torch.float32

        if verbose:
            eprint(f"Loading Florence model: {model_name} on {device} dtype={dtype}")
    ...
    def run(self, image: Image.Image, task: str, max_new_tokens: int, num_beams: int) -> str:
        inputs = self.processor(text=task, images=image, return_tensors="pt")
        inputs = {
            k: (v.to(self.device, self.dtype) if k == "pixel_values" else v.to(self.device))
        ...
        try:
            ...
            text = extract_text_from_florence_parsed(parsed, task)
            if text:
                return clean_model_text(text)
        except Exception:
            pass

        return clean_model_text(generated_text)


def extract_text_from_florence_parsed(parsed, task: str) -> str:
    ...
    if isinstance(value, dict):
        ...
        # For other parsed forms, flatten simple scalar values.
        parts = []
...
def candidate_terms_from_text(text: str, min_len: int) -> List[str]:
    ...
    # Prefer concise noun-ish phrases before single terms. Without POS tagging, this is heuristic.
    for n in (2, 3, 1):
...
def pick_tags(model_text: str, max_tags: int, prefix: str, min_len: int) -> List[str]:
    ...
    seen: Set[str] = set()
    for term in terms:
        # Avoid selecting a single word if a selected phrase already contains it.
        if any(term in chosen.split() for chosen in seen):
            continue
        if term in seen:
            continue
        seen.add(term)
        selected.append(AI_PREFIX + prefix.strip(" /") + "/" + term)
...
def merge_tags(existing: Sequence[str], generated: Sequence[str], low_quality_tag: str, keep_low_quality: bool) -> List[str]:
    ...
    for tag in existing:
        if not keep_low_quality and tag.casefold() == low_quality_tag.casefold():
            continue
        # Replace old ai:prospective tags from previous v4 experiments.
...
def main() -> int:
    ...
    runner = FlorenceRunner(args.model, device, args.verbose)

    repaired = 0
    errors = 0
    no_generated_tags = 0

    for xmp, media, existing_tags, old_caption, tree in tqdm(candidates, desc="Florence repair"):
        try:
            image = prepare_image(media, args.max_side, args.heic_fallback)
            model_text = runner.run(image, args.task, args.max_new_tokens, args.num_beams)
            ...
            if args.verbose or not args.apply:
                ...
                print(f"  florence_text: {model_text}")
